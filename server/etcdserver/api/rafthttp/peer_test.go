...
func TestPeerPick(t *testing.T) {
	tests := []struct {
		...
		{
			true, true,
			&raftpb.Message{Type: new(raftpb.MsgSnap)},
			pipelineMsg,
		},
		{
			true, true,
			&raftpb.Message{Type: new(raftpb.MsgApp), Term: new(uint64(1)), LogTerm: new(uint64(1))},
			streamAppV2,
		},
		{
			true, true,
			&raftpb.Message{Type: new(raftpb.MsgProp)},
			streamMsg,
		},
		{
			true, true,
			&raftpb.Message{Type: new(raftpb.MsgHeartbeat)},
			streamMsg,
		},
		{
			false, true,
