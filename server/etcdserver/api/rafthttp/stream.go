...
var (
	linkHeartbeatMessage     = raftpb.Message{Type: new(raftpb.MsgHeartbeat)}
	linkHeartbeatMessageSize int
)

func init() {
	linkHeartbeatMessageSize = proto.Size(&linkHeartbeatMessage)
}
...
func (cw *streamWriter) run() {
	...
	for {
		...
		case <-heartbeatc:
			...
			unflushed += linkHeartbeatMessageSize
			if err == nil {
				flusher.Flush()
				batched = 0
