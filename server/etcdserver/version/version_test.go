...
// TestUpgradeVersion ensures that Update/unsafeReadStorageVersion work well together.
func TestUpgradeVersion(t *testing.T) {
	tcs := []struct {
		...
		{
			name:             "Backend with 3.5 with metadata keys should be upgraded",
			version:          "",
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer b.Close()
			err := UnsafeUpdateStorageVersion(zap.NewNop(), b.BatchTx())
			...
			//assert.EqualValues(t, tc.expectedMetaKeys, keys)
		})
	}
}
