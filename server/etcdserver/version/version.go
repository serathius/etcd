...
// UnsafeUpdateStorageVersion updates storage version.
func UnsafeUpdateStorageVersion(lg *zap.Logger, tx backend.BatchTx) error {
	...
	switch *v {
	case V3_5:
		lg.Warn("setting storage version", zap.String("storage-version", version.Storage(V3_6.String())))
		// All meta keys introduced in v3.6 should be filled in here.
		unsafeSetStorageVersion(tx, &V3_6)
	case V3_6:
	default:
		lg.Warn("unknown storage version", zap.String("storage-version", version.Storage(v.String())))
	}
	return nil
}
