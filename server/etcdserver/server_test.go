...
func TestApplyRepeat(t *testing.T) {
	...
	cc := &raftpb.ConfChange{Type: new(raftpb.ConfChangeRemoveNode), NodeId: new(uint64(2))}
	ents = []*raftpb.Entry{{
		Index: new(uint64(2)),
		Type:  new(raftpb.EntryConfChange),
...
func diffActions(want, got []testutil.Action) string {
	return cmp.Diff(want, got, protocmp.Transform(), cmpopts.EquateEmpty())
}
...
func TestApplyConfStateWithRestart(t *testing.T) {
	...
	if diff := diffActions(want, got); diff != "" {
	...
	if diff := diffActions(want, got[len(want):]); diff != "" {
		t.Errorf("actions don't match (-want +got):\n%s", diff)
	}
}
...
func TestApplyConfigChangeUpdatesConsistIndex(t *testing.T) {
	...
	cc := &raftpb.ConfChange{Type: new(raftpb.ConfChangeAddNode), NodeId: new(uint64(2)), Context: b}
	ents := []*raftpb.Entry{{
		Index: new(uint64(2)),
		Term:  new(uint64(4)),
		Type:  new(raftpb.EntryConfChange),
...
func TestSnapshotOrdering(t *testing.T) {
	...
	n.readyc <- raft.Ready{Messages: []*raftpb.Message{{Type: new(raftpb.MsgSnap)}}}
	go func() {
...
func TestConcurrentApplyAndSnapshotV3(t *testing.T) {
	...
	for k := 1; k <= 101; k++ {
		...
		if snapMsg.Snapshot.Metadata.GetIndex() == idx {
			idx++
			snapIdx := idx
			snapMsg.Snapshot.Metadata.Index = &snapIdx
