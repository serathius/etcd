	...
	go func() {
		...
		for {
			...
			case rd := <-r.Ready():
				...
				if rd.Snapshot != nil {
					snap = proto.Clone(rd.Snapshot).(*raftpb.Snapshot)
...
func updateCommittedIndex(ap *toApply, rh *raftReadyHandler) {
	...
	if ap.snapshot != nil && ap.snapshot.Metadata != nil && ap.snapshot.Metadata.GetIndex() > ci {
...
func (r *raftNode) processMessages(ms []*raftpb.Message) []*raftpb.Message {
	...
	for i := len(ms) - 1; i >= 0; i-- {
		...
		if m.GetType() == raftpb.MsgSnap {
			...
			mClone := proto.Clone(m).(*raftpb.Message)
			select {
