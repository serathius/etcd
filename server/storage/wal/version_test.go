...
func TestEtcdVersionFromEntry(t *testing.T) {
	...
	confChange := raftpb.ConfChange{Type: new(raftpb.ConfChangeAddLearnerNode)}
	confChangeData := pbutil.MustMarshalMessage(&confChange)

	confChangeV2 := raftpb.ConfChangeV2{Transition: new(raftpb.ConfChangeTransitionJointExplicit)}
