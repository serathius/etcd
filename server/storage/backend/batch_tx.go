...
func (t *batchTxBuffered) Unlock() {
	if t.pending != 0 {
		...
		//
		// Since deletes are now buffered in txWriteBuffer, we no longer need
		// to force an immediate commit on deletes to prevent linearizability
		// breaks. Deletes are safely batched and committed periodically.
		if t.pending >= t.backend.batchLimit {
			t.commit(false)
		}
	}
