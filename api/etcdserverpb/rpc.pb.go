...
type RangeStreamResponse struct {
	...
	//   - header: set only on the first chunk. Clients that do not merge
	//     responses must read the first chunk to observe the revision.
	//   - kvs: each chunk carries a disjoint slice of the result. Concatenating
	//     them in order yields the full key set.
	//   - more, count: set only on the final chunk. Non-merge clients can rely
	//     on io.EOF (end of stream) to detect completion. These fields are
	//     provided for clients that merge all responses into a single
	//     RangeResponse.
